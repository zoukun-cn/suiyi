...
采用与翻译引擎一致的策略模式。每种提示样式实现 `TranslationTipStyle` 接口（`showTranslatingTipStyle` / `updateProgress` / `showTranslatedTipStyle`），由 `TipStyleManager` 统一管理生命周期。用户可在设置中多选独立的提示样式开关。新增样式只需实现接口 + 注册 + 加配置项即可，Manager 和 PageTranslator 无需改动。
...
支持通过 CSS 选择器在特定域名上跳过指定元素（glob URL 匹配）。目前有针对 GitHub 的规则（跳过无障碍跳转链接和视觉隐藏元素）。可通过在 `site-configs.ts` 中添加条目来扩展。
