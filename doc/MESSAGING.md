...
### TRANSLATE_TEXT
→ **Background** ← Content Script / Sidepanel
...
{ type: 'TRANSLATE_TEXT', payload: {
  text: string
  sourceLang?: LanguageCode  // 未填时使用 'auto'
  targetLang?: LanguageCode  // 未填时使用设置中的默认值
  engine?: EngineType        // 未填时使用设置中的默认值
}}

// 响应: TranslateResult
{ translation: string, engine: EngineType }
```
...
### TRANSLATE_PAGE
→ **Content Script** ← Background（右键菜单 / 侧边栏）

指示 page-translator content script 开始翻译当前页面。
...
// 响应: UserSettings
{
  engine: EngineType           // 默认: 'deepseek'
  sourceLang: LanguageCode     // 默认: 'auto'
  targetLang: LanguageCode     // 默认: 'zh-CN'
...
Sidepanel/Options ──TRANSLATE_TEXT──→ Background ──外部 API──→ 翻译结果
       ...
       │                    ┌───────────────┤
       │                    │               │
       │              TRANSLATE_PAGE  PAGE_TRANSLATION_STATUS
       │                    │               │
       │                    ▼               ▲
       │              page-translator.ts ───┘
       │
